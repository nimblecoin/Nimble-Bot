...
class BlockUpdate:
    def __init__(self):
        ...
        self.logger.info('Initialized blockupdater')
    ...
    def check(self):
        ...
        if self.last_fast_check < (datetime.datetime.now() - datetime.timedelta(seconds=self.config['interval'])):
            ...
            self.last_fast_check = datetime.datetime.now()
            req = urllib2.Request(self.config['api_url'] + '&action=getblocksfound&api_key=' + self.config['api_key'] + '&limit=1', headers={'User-Agent': 'Mozilla/5.0'}) 
